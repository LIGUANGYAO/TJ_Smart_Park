...
class Finance extends Admin
{
    ...
    public function index()
    {
        ...
        $content = (new BuilderList())
            ...
            ->keyListItem('corporate_income_tax', '企业所得税(元)')
            ->keyListItem('vat', '增值税(元)')
            ->keyListItem('tax', '总税收(元)')
    ...
    public function edit($id = 0)
    {
        ...
        if (IS_POST) {
            ...
            $content = (new BuilderForm())
                ...
                ->addFormItem('corporate_income_tax', 'text', '企业所得税','必须是数字,默认保留两位小数')
                ->addFormItem('vat', 'text', '增值税','必须是数字,默认保留两位小数')
                ->addFormItem('tax', 'text', '总税收(元)','必须是数字,默认保留两位小数')
