...
use app\park_enterprise_intellectual\model\ParkEnterpriseWebsiteList;
...
//=================================================执行新增企业操作======================================================
            if (IS_POST) {
                ...
                //10,添加企业证书信息
                if ($isZxcqInstall) {
                    $qccReturn = hook('qichachaCertificate', $data['enterprise_name'], true);
                    $qccData = \json_decode($qccReturn[0], true);
                    if (!empty($qccData['Result'])) {
                        $cerData = [];
                        //组装企业证书数据
                        foreach ($qccData['Result'] as $k => $v) {
                            ...
                            $cerData[$k]['type'] = $v['Type'];
                            $cerData[$k]['title'] = $v['Name'];
                            $cerData[$k]['numbering'] = $v['No'];
                            $cerData[$k]['start_date'] = $v['StartDate'];
                            $cerData[$k]['end_date'] = $v['EndDate'];

                        }
                        $cerModel = new ParkEnterpriseEcicertificationList();
                        $cerModel->saveAll($cerData);
                    }
                }

                //11,添加作品著作权数据
                if ($isZxcqInstall) {
                    $qccReturn = hook('qichachaWcopyright', $data['enterprise_name'], true);
                    $qccData = \json_decode($qccReturn[0], true);
                    if (!empty($qccData['Result'])) {
                        $wData = [];
                        //组装作品著作权数据
                ...
                //12,添加软件著作权数据
                if ($isZxcqInstall) {
                    $qccReturn = hook('qichachaScopyright', $data['enterprise_name'], true);
                    $qccData = \json_decode($qccReturn[0], true);
                    if (!empty($qccData['Result'])) {
                        $cData = [];
                        //组装软件著作权数据
                        foreach ($qccData['Result'] as $k => $v) {
                            ...
                            $cData[$k]['title'] = $v['Name'];
                            $cData[$k]['version_no'] = $v['VersionNo'];
                            $cData[$k]['publish_day'] = $v['PublishDate'];
                            $cData[$k]['shortname'] = $v['ShortName'];
                            $cData[$k]['register_no'] = $v['RegisterNo'];
                            $cData[$k]['register_aper_day'] = $v['RegisterAperDate'];
                            $cData[$k]['category'] = $v['Category'];
                        }
                        $cModel = new ParkEnterpriseScopyrightList();
                        $cModel->saveAll($cData);
                    }
                }
                //13,添加网站数据
                if ($isZxcqInstall) {
