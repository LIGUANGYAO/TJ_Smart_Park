...
abstract class ThirdOauth{
	...
	public function __construct($token = null){
		...
		if(empty($config[$platform.'Key']) || empty($config[$platform.'Secret'])){
			throw new \Exception('请配置您申请的APP_KEY和APP_SECRET');
		} else {
			$this->AppKey    = $config[$platform.'Key'];
			$this->AppSecret = $config[$platform.'Secret'];
     ...
     */
    public static function getInstance($platform, $token = null) {
    	$name = ucfirst(strtolower($platform));
    	$class = "\\plugins\\ThirdLogin\\EacooSDK\\sdk\\" . $name ;
	...
	public function config(){
		...
		$this->Callback = "http://".$_SERVER ['HTTP_HOST'].plugin_url('ThirdLogin/Login/callback',['platform'=>strtolower($this->Platform)]);
